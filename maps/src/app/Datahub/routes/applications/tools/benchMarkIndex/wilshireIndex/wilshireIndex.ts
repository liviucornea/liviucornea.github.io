...
import {WilshireIndexControlConfig} from "./wilshireIndex.config";
...
export class BmiWilshireIndex {
    @ViewChild(TabBuilder) tabBuilderControl: TabBuilder;
    tabsListData: Array<any> = [];
    tabControlConfig: any = WilshireIndexControlConfig;
    ...
    showReport() {
        ...
        var spSelected = this.formBuilder.data.find(x=> x.name == "WilshireSubIndex").val;
        ...
        this.apiService.getArrayFromQuery("Benchmark_GetWilshireIndex", JSON.stringify({
            ...
            error => {
                this.alert.error("Benchmark_GetWilshireIndex : async error #" + error.status);
            },
            () => {
            }
        );
