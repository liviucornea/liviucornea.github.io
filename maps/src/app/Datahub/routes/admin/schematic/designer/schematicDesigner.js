...
var SchematicDesigner = (function (_super) {
    ...
    SchematicDesigner.prototype.buildConfigValueTypesVM = function () {
        ...
        this.allConfigValueTypes.forEach(function (x) {
            if (x.UnitId === unitId) {
                var newObj = new schematicDesignerUtils_1.ConfigValueType(x.ConfigValueTypeId, x.UnitId, x.LookupKey, x.Template ? x.Template : '', x.IsMandatory, x.DefaultConfigValue);
    ...
    SchematicDesigner.prototype.configValueTypeSelected = function (selectedValue, step) {
        ...
        var lcConfigValFound = this.allConfigValueTypesVM.find(function (x) {
        ...
        if (lcConfigValFound) {
            try {
                if (!lcConfigValFound.template || lcConfigValFound.template.length == 0) {
                    ...
                    var type = JSON.parse(lcConfigValFound.template)['#Type#'];
                    if (type) {
                        step.selectedConfigType = lcConfigValFound;
                        if (type.toUpperCase() == 'ARRAYLIKE') {
                            step.selectedConfigValue = '[[' + schematicDesignerUtils_1.jsonToDoc(lcConfigValFound.template, step) + ']]';
    ...
    SchematicDesigner.prototype.expandAll = function () {
        for (var _i = 0, _a = this.allStepsVM; _i < _a.length; _i++) {
            var step = _a[_i];
            step.isCollapsed = false;
        }
    };
    SchematicDesigner.prototype.collapseAll = function () {
        for (var _i = 0, _a = this.allStepsVM; _i < _a.length; _i++) {
            var step = _a[_i];
            step.isCollapsed = true;
        }
    };
    SchematicDesigner.prototype.isAnyExpanded = function () {
        var expandedSteps = this.allStepsVM.find(function (x) {
