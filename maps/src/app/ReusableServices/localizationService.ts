...
export class LocalizationService {
    ...
    setDefaultLanguge()
    {
        this.getLocalizationLanguagesList().subscribe(
            res=>{
                ...
                {
                    var selectedLang = this.languageList.find(p=>p.LangCode.toLowerCase().substring(0,2) =='en');
                    if(selectedLang)
                    {
    ...
    getLocalizedValueDescription(valueSet)
    {
        ...
        {
            var tempLocalizationValue = this.localizationResourcesList.find(p=>p.ValueSetDescription.toLowerCase() == valueSet.toString().toLowerCase());
