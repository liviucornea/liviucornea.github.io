...
import {FormBuilderInlineChildren} from "../formBuilderInlineChildren/formBuilderInlineChildren";



@Component({
    templateUrl: "app/ReusableComponents/formBuilderTreeView/formBuilderTreeView.html",
    selector: 'formBuilder',
    directives: [RadioControlValueAccessor,FormBuilderInline, FormBuilderInlineChildren]
})

export class FormBuilderTreeView {
    ...
    constructor(alert:AlertService, elementRef:ElementRef, dcl:DynamicComponentLoader, private appSettingsService:AppSettingsService, private intFormSer:InterFormsService,private vmMat: matrixService) {
        this.alert = alert;
    ...
    injectDataAndConfigAndChildren(data:any, children:any, config:any, pageType:string) {
        this.gridSettings = config;
        this.PageType = pageType;
        this.editViewRowDataTable = data;
    ...
    saveClicked() {
        for (var colInd in this.editViewRowDataTable) {
            ...
            var type = this.gridSettings.ColumnConfiguration.find(x=> {
                return x.dbColumnName === column.name
            });
    ...
    childClicked(child) {
        this.children.forEach(x=>x.visible=false);
        child.visible=true;
    }
}
