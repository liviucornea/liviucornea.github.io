import {Injector,Injectable, Output, EventEmitter} from "angular2/core";
...
export class crudService {
    ...
    updateInlineRecord(columns:any, gridSettings:any, pageName:any, httpProxy:any) {
        for (var colInd in columns) {
            ...
            var type = gridSettings[column.name];
        ...
        var primaryColumnName = this.getPrimaryColumnName(gridSettings);
        var pluginValue = this.buildJSONObject(columns,primaryColumnName);
    ...
    delete(columns:any, pageName, primaryColumnName:any, httpProxy:any){
        var data=this.buildJSONObject(columns,primaryColumnName )
       if (httpProxy) {
